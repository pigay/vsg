...
# Modified by Pierre Gay <gay-pierre@wanadoo.fr> to fit my needs
# (mainly to be able to scan files with a lot of "@"...)
...
pointer_pat = re.compile('(' + obj_name_pat + ')\*$')
...
def write_func(fp, name, ret, args):
    ...
    # we now check the name of the object return matches the _new one
    m = func_new_pat.match(name)
    mp = pointer_pat.match(ret)
    if mp and m:
        fname = m.group(1).replace('_','')
        cname = mp.group(1)
	if cname.lower() == fname:
