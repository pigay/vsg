...
void exterior_move (VsgVector3d *point, gpointer user_data);
...
static VsgMatrix4d _rotate;

static guint _npoints = 8;
...
VsgVector3d *vsg_vector3d_alloc (gboolean resident, GPtrArray *ptarray)
{
  VsgVector3d *ret;
  ret = vsg_vector3d_new (0., 0., 0.);
...
void vsg_vector3d_destroy (VsgVector3d *v, gboolean resident,
                           GPtrArray *ptarray)
{
  if (resident)
...
static void points1 (guint *npoints, GPtrArray *array)
{
  ...
  guint n = sizeof (pts) / sizeof (VsgVector3d);
  gint i;

  for (i=0; i<n; i++)
  {
    VsgVector3d *point = vsg_vector3d_alloc (TRUE, array);
    vsg_vector3d_copy (&pts[i], point);
  }
  ...
  for (i=0; i<*npoints; i++)
    {
      ...
      point->z = g_rand_double_range (_rand, -1., 1.);
    }

}

gboolean old_move (VsgPRTree3d *tree, VsgPoint3 point, GFunc move_func,
		   gpointer move_data)
{
  gboolean found = vsg_prtree3d_remove_point (tree, point);

  if (! found) return FALSE;

  move_func (point, move_data);

  vsg_prtree3d_insert_point (tree, point);

  return TRUE;
}

void random_move (VsgVector3d *point, gdouble *scale)
{
  point->x += g_rand_double_range (_rand, -*scale, *scale);
  point->y += g_rand_double_range (_rand, -*scale, *scale);
  point->z += g_rand_double_range (_rand, -*scale, *scale);

  if (_verbose) test_printerr ("new pos: %f %f %f\n", point->x, point->y,
                               point->z);
}
...
void exterior_move (VsgVector3d *point, gpointer user_data)
{
  ...
  if (point->z != 0.) point->z *= (1. + 1./fabs (point->z));

  if (_verbose) test_printerr ("new pos: %f %f %f\n", point->x, point->y,
                               point->z);
}

void rotate_move (VsgVector3d *point, VsgMatrix4d *rotation)
{
  vsg_matrix4d_vecmult (rotation, point, point);

  if (_verbose) test_printerr ("new pos: %f %f %f\n", point->x, point->y,
                               point->z);
}

void zero_move (VsgVector3d *point, VsgMatrix4d *rotation)
{
}

void point_checksum (VsgVector3d *point, GChecksum *checksum)
{
  g_checksum_update (checksum, (guchar *) point, sizeof (VsgVector3d));

  if (_verbose)
    {
      test_printerr ("");
      vsg_vector3d_write (point, stderr);
      g_printerr ("\n");
    }
}

#define KEY3D_CHKSUM_SIZE  (3*sizeof (guint64) + sizeof (guint8))

/* tree traversal function that compute checksum for a tree */
static void traverse_checksum (VsgPRTree3dNodeInfo *node_info,
			       GChecksum *checksum)
{
  g_checksum_update (checksum, (guchar *) &node_info->id, KEY3D_CHKSUM_SIZE);
  if (_verbose)
    {
      test_printerr ("");
...
static GChecksum *vsg_prtree3d_checksum (VsgPRTree3d *tree, GChecksumType type)
{
  GChecksum *checksum = g_checksum_new (type);

  vsg_prtree3d_traverse (tree, G_PRE_ORDER,
			 (VsgPRTree3dFunc) traverse_checksum, checksum);
...
void vsg_vector3d_migrate_pack (VsgVector3d *v, VsgPackedMsg *pm,
                                 gpointer user_data)
{
  vsg_packed_msg_send_append (pm, v, 1, VSG_MPI_TYPE_VECTOR3D);
}

void vsg_vector3d_migrate_unpack (VsgVector3d *v, VsgPackedMsg *pm,
                                  gpointer user_data)
{
  vsg_packed_msg_recv_read (pm, v, 1, VSG_MPI_TYPE_VECTOR3D);
}
...
static VsgPRTreeParallelConfig pconfig = {
  ...
  {(VsgMigrableAllocDataFunc) NULL, NULL,
   ...
   {(VsgMigrablePackDataFunc) vsg_vector3d_migrate_pack, NULL,
    (VsgMigrableUnpackDataFunc) vsg_vector3d_migrate_unpack, NULL,
    NULL, NULL},
  },
  {},{},
  MPI_COMM_WORLD,
	  ...
	  if (g_ascii_strncasecmp (arg, "random", 6) == 0)
	      ...
	      _move_data = NULL;
           ...
           else if (g_ascii_strncasecmp (arg, "rotate", 6) == 0)
            {
              if (strlen (arg) > 6)
                _theta = atof (arg+6);
              vsg_matrix4d_rotate_z (&_rotate, _theta);
...
gint main (gint argc, gchar ** argv)
{
  ...
  for (i=0; i<_npoints; i++)
    {
      VsgVector3d *ptref = vsg_vector3d_alloc (TRUE, pointsref_array);
      vsg_vector3d_copy ((VsgVector3d *) g_ptr_array_index (points_array, i),
                         ptref);
    }
  ...
  tree =
    vsg_prtree3d_new_full (&lb, &ub,
                           ...
                           NULL, _maxbox);
  treeref = vsg_prtree3d_clone (tree);

#ifdef VSG_HAVE_MPI
  if (_mpi)
    {
      pconfig.point.alloc = (VsgMigrableAllocDataFunc) vsg_vector3d_alloc;
      pconfig.point.alloc_data = points_array;
      pconfig.point.destroy = (VsgMigrableDestroyDataFunc) vsg_vector3d_destroy;
      pconfig.point.destroy_data = points_array;

      prefconfig.point.alloc = (VsgMigrableAllocDataFunc) vsg_vector3d_alloc;
      prefconfig.point.alloc_data = pointsref_array;
      prefconfig.point.destroy = (VsgMigrableDestroyDataFunc) vsg_vector3d_destroy;
      prefconfig.point.destroy_data = pointsref_array;

      vsg_prtree3d_set_parallel (tree, &pconfig);
      vsg_prtree3d_set_parallel (treeref, &prefconfig);
    }
#endif
  ...
  for (i=0; i<_npoints; i++)
      ...
      if (_verbose)
	{
	  ...
	  vsg_vector3d_write (g_ptr_array_index (points_array, i), stderr);
	  test_printerr ("\n");
	}

      vsg_prtree3d_insert_point (tree, g_ptr_array_index (points_array, i));
      vsg_prtree3d_insert_point (treeref,
  ...
  if (_mpi)
    {
      ...
      vsg_prtree3d_migrate_flush (treeref);

      vsg_prtree3d_distribute_contiguous_leaves (tree);
      vsg_prtree3d_distribute_contiguous_leaves (treeref);
    }
#endif
  ...
  {
    vsg_prtree3d_move_point (tree, g_ptr_array_index (points_array, i), _move,
                             _move_data);
  }
#ifdef VSG_HAVE_MPI
  if (_mpi)
  ...
  checksum = vsg_prtree3d_checksum (tree, G_CHECKSUM_MD5);
  checksumref = vsg_prtree3d_checksum (treeref, G_CHECKSUM_MD5);
...
#endif
      test_printerr ("-------------------------------\n");
      vsg_prtree3d_write (tree, stderr);
#ifdef VSG_HAVE_MPI
      if (_mpi)
        MPI_Barrier (MPI_COMM_WORLD);
#endif
      test_printerr ("-------------------------------\n");
      vsg_prtree3d_write (treeref, stderr);
    }
  else if (_verbose)
    {
      test_printerr ("ref = \"%s\"\n", chkstrref);
  ...
  for (i=0; i<points_array->len; i++)
    {
      vsg_prtree3d_remove_point (tree, g_ptr_array_index (points_array, i));
      vsg_prtree3d_remove_point (treeref,
                                 g_ptr_array_index (pointsref_array, i));
    }

  /* destroy the trees */
  vsg_prtree3d_free (tree);
  vsg_prtree3d_free (treeref);

  for (i=0; i<points_array->len; i++)
    vsg_vector3d_destroy (g_ptr_array_index (points_array, i), FALSE,
                          points_array);

  for (i=0; i<pointsref_array->len; i++)
    vsg_vector3d_destroy (g_ptr_array_index (pointsref_array, i), FALSE,
                          pointsref_array);
