...
void pt_migrate_unpack (gpointer pt, VsgPackedMsg *pm,
                        gpointer user_data)
{
  vsg_packed_msg_recv_read (pm, &((Pt *) pt)->vector, 1, VSG_MPI_TYPE_VECTOR3D);
...
gint main (gint argc, gchar ** argv)
{
  ...
  vsg_prtree3d_set_nf_isleaf (tree, _nf_isleaf_virtual_maxbox,
                              &_virtual_maxbox);

  vsg_parallel_vtable_set (&pconfig.node_data,
                           node_counter_alloc, NULL,
                           node_counter_destroy, NULL);
  vsg_prtree3d_set_node_data_vtable (tree, &pconfig.node_data);

  vsg_parallel_vtable_set (&prefconfig.node_data,
                           node_counter_alloc, NULL,
                           node_counter_destroy, NULL);
  vsg_prtree3d_set_node_data_vtable (treeref, &prefconfig.node_data);
  ...
  vsg_prtree3d_near_far_traversal (treeref, (VsgPRTree3dFarInteractionFunc) _far,
                                   (VsgPRTree3dInteractionFunc) _near,
                                   &ret);
  vsg_prtree3d_traverse (treeref, G_PRE_ORDER, (VsgPRTree3dFunc) _down, NULL);
  ...
  if (_mpi)
    {
      ...
      vsg_prtree3d_distribute_concentrate (tree, 0);
      vsg_prtree3d_distribute_concentrate (treeref, 0);
    }
#endif
      ...
      if (pt->count != ptref->count)
        {
          ...
          vsg_vector3d_write (&ptref->vector, stderr);
          g_printerr ("]=%ld\n", ptref->count);
        }
      else if (_verbose)
        {
