...
static void _send_pending_forward_node (VsgPRTree3@t@ *tree,
                                        VsgNFConfig3@t@ *nfc,
                                        VsgNFProcMsg *nfpm,
                                        gint proc)
{
...
static void _do_send_backward_node (VsgPRTree3@t@ *tree,
                                    VsgNFConfig3@t@ *nfc,
                                    VsgNFProcMsg *nfpm,
                                    VsgPRTree3@t@Node *node,
                                    VsgPRTreeKey3@t@ *id,
...
/*                           nfc->rk, nfpm->dropped_visitors, wv->src); */
  ...
  vsg_packed_msg_send_append (msg, id, 1, VSG_MPI_TYPE_PRTREE_KEY3@T@);
  _node_pack_and_destroy (node, &npd);
  vsg_prtree3@t@node_free (node, &tree->config);
...
static void _send_pending_backward_node (VsgPRTree3@t@ *tree,
                                         VsgNFConfig3@t@ *nfc,
                                         VsgNFProcMsg *nfpm,
                                         gint proc)
{
...
static void _propose_node_backward (VsgPRTree3@t@ *tree,
                                    VsgNFConfig3@t@ *nfc,
                                    gint proc,
                                    WaitingVisitor *wv)
{
 ...
 */
static gboolean _compute_visiting_node (VsgPRTree3@t@ *tree,
                                        VsgNFConfig3@t@ *nfc,
                                        WaitingVisitor *wv)
{
  NIAndFuncs niaf;
  ...
  while (i > 0)
    {
      ...
      if (flag)
      ...
      else
        {
          ...
          if (! vsg_prtree3@t@_nf_check_receive (tree, nfc, MPI_ANY_TAG, FALSE))
            g_usleep (1);
        }
    }
