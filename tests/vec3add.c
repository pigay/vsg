...
gint main (gint argc, gchar ** argv)
{
  ...
  vsg_vector3d_add (&v, &w, &res);
  vsg_vector3d_set (&ref, 1.1, 2.2, 3.3);
  ret += vcheck ("[1, 2, 3] + [0.1, 0.2, 0.3] == [1.1, 2.2, 3.3]", &res, &ref);

  vsg_vector3d_set (&v, 1.2e-2, 0, 0);
  vsg_vector3d_set (&w, 1.2e-3, 0, 0);
  vsg_vector3d_add (&v, &w, &res);
  vsg_vector3d_set (&ref, 0.0132, 0, 0);
  ret += vcheck ("[1.2e-2, 0, 0] + [1.2e-3, 0, 0] == [0.0132, 0, 0]", &res,
                 &ref);
