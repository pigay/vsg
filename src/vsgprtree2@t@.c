...
static VsgPRTree2@t@Node *_root_extend (const VsgVector2@t@ *lbound,
                                      const VsgVector2@t@ *ubound,
...
_z_order_data (gpointer key, gint *children, gpointer *children_keys,
               gpointer user_data)
{
  static const gint zorder[CN] = {0, 1, 2, 3};
                         ...
                         guint max_point)
{
  ...
  vsg_prtree2@t@_set_nf_isleaf (prtree2@t@, NULL, NULL);
...
VsgPRTree2@t@ *vsg_prtree2@t@_clone (VsgPRTree2@t@ *prtree2@t@)
{
  ...
  res->config.nf_isleaf = prtree2@t@->config.nf_isleaf;
  res->config.nf_isleaf_data = prtree2@t@->config.nf_isleaf_data;
                                   ...
                                   VsgPoint2 point, VsgPRTreeKey2@t@ *extk)
{
  ...
  while (CALL_POINT2@T@_LOC (config, point, lbound) != VSG_LOC2_NE ||
         CALL_POINT2@T@_LOC (config, point, ubound) != VSG_LOC2_SW)
    {
      ...
      prtree2@t@->node = _root_extend (&new_lbound,
                                       &new_ubound,
                                       prtree2@t@->node,
                                       loc,
                                       config);
